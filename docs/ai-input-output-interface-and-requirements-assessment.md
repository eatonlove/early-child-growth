...
> 对应版本：童迹 3.0 生产代码 `798c130`  
> AI Provider：千问 `qwen3.7-plus`，故障时可明确回退为本地模拟规则  
> 核心原则：AI 只生成教师审核用草稿，不作诊断、排名、综合评分或自动发布

## 1. 先说结论

童迹 3.0 当前有三类真实 AI 能力：

1. **单次游戏观察分析**：分析教师白描、幼儿原话、教师识别、教师应答、年龄段知识卡，以及获得授权的图片或视频画面。
2. **周期报告生成**：汇总指定周期内教师已经采用的观察、AI 分析和应答效果，生成教师版或家长版报告草稿。
3. **游戏课程草案生成**：对达到证据门槛的同主题连续观察生成课程线索和可编辑课程草案。

当前系统已经满足“标准化 AI 输出、年龄段知识参照、媒体画面分析、教师最终决策、报告与课程草案生成”等核心要求，但**还没有完全满足原始完整需求**。主要缺口是：

- AI 分析只能整份采用或放弃，不能逐条采用、修改、拒绝或标记待验证。
- 部分已经生成并保存的 AI 字段没有在前端展示，证据链没有完整呈现给教师。
- 视频只分析画面，不处理音轨，也没有真实语音转写和事件自动分段。
- 单次分析不读取该幼儿历史观察，不能在分析阶段直接进行跨时间成长对比。
- 周期报告可以基于一条已采用观察生成，尚未强制多时间点证据门槛。
- 课程聚类目前按完全相同的主题文字分组，不是语义兴趣聚类。

因此，当前版本适合定位为：**可正式使用的 AI 辅助循证观察基础版**，但还不是“完整科研级、逐主张审核、自动纵向比较”的终局版本。
...
flowchart LR
    ...
    H --> I{教师审核}
    I -->|采用| J[进入成长轨迹并创建应答行动]
    I -->|放弃| K[保留审计记录，不进入后续成果]
    J --> L[实施、复察、验证效果]
    J --> M[周期报告]
    J --> N[课程线索扫描]
```
...
`观察事实 → 教师识别 → 教师应答 → AI 第二视角 → 教师采用或放弃`
...
| 处理 AI 结果 | `POST /api/analyses/:id/decision` | `decision`、`note` | 整份采用或放弃 |
| 生成周期报告 | `POST /api/reports/generate` | 幼儿、班级、报告类型、周期 | 汇总已采用证据生成草稿 |
| 扫描课程线索 | `POST /api/curriculum-clues/scan` | 班级 ID | 按主题和时间点聚合已采用观察 |
...
- 观察状态变为 `adopted`，可以进入成长轨迹、报告和课程扫描。
- 系统自动取 `responseSuggestions.experience[0]` 创建一条“经验支持”行动。
...
- 只要周期内存在 1 条已采用观察就可以生成，没有强制至少 2 个时间点。
...
当前课程 AI 不直接接收教师客观白描、幼儿原话、媒体内容、已采用 AI 分析全文或知识卡，只接收教师识别和应答，因此课程的证据细节仍然偏少。
...
| 所有判断回链原始证据 | 数据层有证据 ID 校验 | 部分满足，前端展示不完整，语义真实性仍需人工核验 |
| 教师逐条采用、修改、拒绝、待验证 | 目前只能整份采用或放弃 | **不满足** |
| AI 不覆盖教师原始判断 | 教师识别和应答独立保存并由后端原文覆盖 | 满足 |
| 应答可实施、复察和判断效果 | 已有完整状态机 | 满足 |
| 教师自由选择 AI 建议形成应答 | 自动只取第一条经验建议 | **部分满足** |
| 精确对比幼儿跨时间成长 | 报告可汇总多次观察，但单次分析不读取历史，也无结构化变化计算 | **部分满足** |
| 月度、学期、自定义周期报告 | 支持自定义日期、教师版和家长版 | 基本满足 |
| 报告必须基于连续证据 | 当前只强制至少 1 条已采用观察 | **部分满足** |
| 教师修改报告后发布 | 有审核发布状态，但没有报告内容编辑 | **不满足** |
| 从持续兴趣生成课程 | 有多幼儿/多时间点门槛和课程草案 | 基本满足 |
| 相近兴趣自动聚类 | 目前按完全相同的主题字符串聚类 | **不满足语义聚类** |
...
1. 将整份采用改为主张级审核：每条事实、解释、假设和应答分别支持采用、修改、拒绝、待验证。
2. 在页面完整显示 `evidenceIds`、解释限制、置信度、下一轮观察和证据缺口。
3. 证据 ID 点击后打开对应白描、原话、图片或视频时间段。
4. 教师修改后保存“AI 原文、教师修订稿、修改原因和版本”。

### P1：视频和纵向成长分析

1. 接入视频音轨 ASR、说话人标注和教师确认转写。
2. 视频按时间码分段，事实可回链到具体片段。
3. 单次分析增加该幼儿的历史相关观察摘要，但控制最小必要范围。
4. 增加跨时间变化对象：行为维度、前次证据、本次证据、支持措施、变化描述、仍需验证内容。

### P1：应答与报告

1. 让教师从三类 AI 建议中勾选一条或多条建立支持行动，不再固定取第一条经验建议。
2. 报告至少要求 2 个时间点；“成长变化”逐条绑定观察 ID。
3. 报告草稿开放编辑并记录审核意见、修改版本和发布版本。

### P2：课程生成

1. 将完全相同主题分组升级为可解释的语义聚类，并允许教师合并或拆分主题。
2. 课程 AI 输入增加脱敏白描摘要、幼儿问题、已采用分析和知识卡依据。
3. 课程所有生成字段开放编辑，并逐部分关联证据。
...
5. **最终进入成长、报告和课程的内容是否经过教师明确审核？** 当前只能整份审核，这是下一阶段最优先的改进点。
...
> 一份以当前证据和对应年龄段知识卡为边界的结构化 AI 草稿，包括事实、可能解释、待验证假设、经验与兴趣、证据缺口、发展参照、支持建议、下一轮观察重点和风险提示；教师采用后才进入应答追踪、周期报告和课程生成。

...
function SimpleAnalysisBoard({
  ...
  return (
    <div className="simple-analysis-board">
      ...
      {result.externalSupportReferences.length > 0 && <Panel title="公开资料补充" subtitle="联网资料只用于启发支持方法，不作为幼儿发展判断证据；检索未发送幼儿身份资料。"><div className="external-support-list">{result.externalSupportReferences.map((item) => <a href={item.url} target="_blank" rel="noreferrer" key={item.url}><strong>{item.title}</strong><span>{item.source}</span><p>{item.appliedSuggestion}</p></a>)}</div></Panel>}

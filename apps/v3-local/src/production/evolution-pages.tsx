...
import {
  ...
  FileInput, FileText, FileVideo, Plus, RefreshCcw, Save, Sparkles, Trash2, Upload, Users,
} from "lucide-react";
...
const focusCategoryLabel: Record<string, string> = { materials_tools: "材料与工具", cognition_experience: "认知与经验", social_experience: "交往与经验" };
...
const importStatusLabel: Record<RemoteObservationImport["status"], string> = {
  pending_upload: "等待上传", extracting: "正在解析", needs_review: "待教师校对", confirmed: "已形成正式观察", failed: "解析失败",
};
...
type SubjectDraft = { childId: string; role: "primary" | "participant" | "incidental"; contextualFeature: string; evidenceAnchors: string[] };
const emptyObservation = () => ({
  classroomId: "", templateId: "", sourceImportId: "", observerName: "", observerIds: [] as string[], occurredAt: new Date().toISOString().slice(0, 16), durationMinutes: 20,
  scene: "建构区", theme: "", organizationStage: "process", observationFocusCategory: "materials_tools", observationFocus: "材料选择与使用",
...
export function normalizeAnalysisResultForView(rawResult: RemoteAnalysis["structured_result"]) {
  return {
    ...
    result: {
      ...
      externalSupportReferences: rawResult.externalSupportReferences ?? [],
    },
  };
}
...
function SimpleAnalysisBoard({
  ...
  return (
    <div className="simple-analysis-board">
      ...
      <Panel title="连续观察对比" subtitle="只比较该幼儿更早且已由教师确认采用的观察，不与其他幼儿比较。">
        <div className="historical-comparison-head"><Badge tone={result.historicalComparison.timePointCount > 0 ? "green" : "gray"}>{result.historicalComparison.timePointCount}个历史时间点</Badge><span>{result.historicalComparison.caution}</span></div>
        {result.historicalComparison.changes.map((item) => <article className="historical-change" key={`${item.dimension}-${item.content}`}><strong>{item.dimension}</strong><p>{item.content}</p><small>历史证据：{item.previousEvidenceIds.join("、")} · 本次证据：{item.currentEvidenceIds.join("、")} · 置信度{Math.round(item.confidence * 100)}%</small></article>)}
        {result.historicalComparison.stablePatterns.map((item) => <article className="historical-change" key={item.content}><strong>持续出现的线索</strong><p>{item.content}</p><small>{item.evidenceIds.join("、")}</small></article>)}
      </Panel>
      ...
      <Panel title="完整应答方案" subtitle="活动、材料、经验支持均来自本次证据；教师确认AI分析后可选择一套进入实施与复察。">
        <div className="simple-response-list detailed">{responsePlans.map((plan) => <article key={plan.id}><div className="response-plan-heading"><div><Badge tone={tone(plan.status)}>{statusLabel[plan.status] ?? plan.status}</Badge><h3>{plan.title}</h3></div>{plan.status === "suggested" && <button className="btn btn-primary" disabled={busy || analysis.decision !== "adopted"} onClick={() => void run(() => remoteApi.selectResponsePlan(plan.id))}>采用该应答</button>}</div><p>{plan.rationale}</p><div className="response-support-grid"><section><h4>活动支持</h4><p><b>{plan.activity_support.activityName}</b></p><small>时机：{plan.activity_support.timing} · 时长：{plan.activity_support.suggestedDuration}</small><p>{plan.activity_support.objective}</p><ol>{plan.activity_support.steps.map((step) => <li key={step}>{step}</li>)}</ol><small>教师角色：{plan.activity_support.teacherRole}</small></section><section><h4>材料支持</h4>{plan.material_support.materials.map((material) => <p key={`${material.name}-${material.variable}`}><b>{material.name}</b> · {material.quantity || "按现场需要"}<br /><small>变量/用法：{material.variable || "由幼儿自主选择"}</small></p>)}<p>摆放：{plan.material_support.placement}</p><p>用途：{plan.material_support.purpose}</p>{plan.material_support.safetyNotes.map((note) => <small key={note}>• {note}</small>)}</section><section><h4>经验支持</h4>{plan.experience_support.suggestedQuestions.map((question) => <p key={question}>“{question}”</p>)}<p>参与方式：{plan.experience_support.participationMode}</p><p>示范方式：{plan.experience_support.demonstration || "优先不示范"}</p><small>退出条件：{plan.experience_support.withdrawalCondition}</small></section></div><footer><b>复察切口：</b>{plan.observation_cut}<br /><b>调整条件：</b>{plan.adjustment_condition}</footer></article>)}</div>
      </Panel>
      {result.externalSupportReferences.length > 0 && <Panel title="公开资料补充" subtitle="联网资料只用于启发支持方法，不作为幼儿发展判断证据；题名、作者和网址来自已验证检索结果。"><div className="external-support-list">{result.externalSupportReferences.map((item) => <a href={item.url} target="_blank" rel="noreferrer" key={item.url}><strong>{item.title}</strong><span>{item.authors?.length ? `${item.authors.join("、")} · ` : ""}{item.publication || item.source}{item.publicationYear ? ` · ${item.publicationYear}` : ""}</span><small>{item.sourceType === "academic" ? "学术资料" : item.sourceType === "policy" ? "政策文件" : item.sourceType === "institutional" ? "机构资料" : "实践资料"}{item.verified ? " · 来源已核验" : ""}</small><p>{item.appliedSuggestion}</p></a>)}</div></Panel>}
...
export function RemoteObservationV32Page({ user }: { user: RemoteUser }) {
  ...
  const filteredClassChildren = classChildren.filter((child) => !childSearch.trim() || `${child.display_name} ${child.internal_code}`.toLowerCase().includes(childSearch.trim().toLowerCase()));
  const childMap = useMemo(() => new Map(children.map((child) => [child.id, child])), [children]);
  const archiveYears = [...new Set(classrooms.map((item) => item.academic_year))];
  const archiveSemesters = [...new Set(classrooms.filter((item) => !archiveFilter.academicYear || item.academic_year === archiveFilter.academicYear).map((item) => item.semester))];
  const archiveClassrooms = classrooms.filter((item) => (!archiveFilter.academicYear || item.academic_year === archiveFilter.academicYear) && (!archiveFilter.semester || item.semester === archiveFilter.semester));
  const visibleObservations = observations.filter((item) => {
    if (archiveFilter.classroomId && item.classroom_id !== archiveFilter.classroomId) return false;
    if (!archiveFilter.classroomId && !archiveClassrooms.some((classroom) => classroom.id === item.classroom_id)) return false;
  ...
  const load = async () => {
    ...
    setForm((current) => ({ ...current, observerName: current.observerName || user.displayName, classroomId: current.classroomId || classes.items.find((item) => item.status === "active")?.id || "" }));
  };
  ...
  }, [archiveFilter.classroomId, archiveFilter.academicYear, archiveFilter.semester, archiveFilter.year, archiveFilter.month, observations]);
  useEffect(() => {
  ...
  useEffect(() => {
    const importedCategory = importResult?.extracted_fields.observationFocusCategory;
    if (importedCategory) setForm((current) => ({ ...current, observationFocusCategory: importedCategory }));
  }, [importResult?.id]);
  useEffect(() => {
    if (!classChildren.length) return setForm((current) => ({ ...current, subjects: [] }));
    setForm((current) => current.subjects.some((subject) => classChildren.some((child) => child.id === subject.childId)) ? current : { ...current, subjects: [{ childId: classChildren[0].id, role: "primary", contextualFeature: "", evidenceAnchors: [] }] });
  ...
  const toggleSubject = (childId: string) => setForm((current) => {
    ...
    return { ...current, subjects: [...current.subjects, { childId, role: "participant", contextualFeature: "", evidenceAnchors: [] }] };
  });
  const submit = (event: FormEvent) => { event.preventDefault(); void withBusy(async () => {
    const created = await remoteApi.createObservation({
      ...
      observerName: form.observerName, observerIds: form.observerIds, groupContext: form.groupContext, unlistedParticipantCount: Number(form.unlistedParticipantCount),
      subjects: form.subjects, occurredAt: new Date(form.occurredAt).toISOString(), durationMinutes: Number(form.durationMinutes),
      scene: form.scene, theme: form.theme, organizationStage: form.organizationStage, observationFocusCategory: form.observationFocusCategory, observationFocus: lines(form.observationFocus),
    ...
    setEntry(null); setFiles([]); setImportFile(null); setImportResult(null); setForm({ ...emptyObservation(), observerName: user.displayName, classroomId: form.classroomId });
  ...
  const importDocument = () => void withBusy(async () => {
    ...
    const matched = (fields.subjects ?? []).map((subject, index) => {
      ...
      return child ? { childId: child.id, role: index === 0 ? "primary" as const : subject.role === "primary" ? "participant" as const : subject.role, contextualFeature: subject.contextualFeature, evidenceAnchors: [] as string[] } : null;
    }).filter((item): item is SubjectDraft => Boolean(item));
    setForm((current) => ({ ...current, sourceImportId: result.item.id, observerName: fields.observerName || current.observerName || user.displayName, occurredAt: localDateTime(fields.occurredAtText || "") || current.occurredAt, scene: fields.scene || current.scene, theme: fields.theme || current.theme,
      organizationStage: fields.organizationStage || current.organizationStage, observationFocusCategory: fields.observationFocusCategory || current.observationFocusCategory, subjects: matched.length ? matched : current.subjects,
  ...
  return <div className="page remote-page evo-page">
    ...
    <Panel className="observation-archive-toolbar" title="班级观察档案" subtitle="按班级与学期归集连续观察，可继续缩小到指定年份、月份并直接导出Word。">
      <div className="archive-filter-grid">
        <label><span>学年</span><select value={archiveFilter.academicYear} onChange={(event) => setArchiveFilter({ ...archiveFilter, academicYear: event.target.value, semester: "", classroomId: "" })}><option value="">全部学年</option>{archiveYears.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label><span>学期</span><select value={archiveFilter.semester} onChange={(event) => setArchiveFilter({ ...archiveFilter, semester: event.target.value, classroomId: "" })}><option value="">全部学期</option>{archiveSemesters.map((item) => <option key={item}>{item}</option>)}</select></label>
    ...
    <div className="remote-observation-layout">
      <Panel className="remote-observation-list"><div className="remote-list-head"><strong>观察记录</strong><span>{visibleObservations.length}条</span></div>{visibleObservations.map((item) => <button className={selected === item.id ? "selected" : ""} key={item.id} onClick={() => setSelected(item.id)}><span>{item.scene.slice(0, 1)}</span><div><strong>{item.title}</strong><small>{item.scene} · {new Date(item.occurred_at).toLocaleDateString("zh-CN")}</small><Badge tone={tone(item.status)}>{statusLabel[item.status] ?? item.status}</Badge></div></button>)}</Panel>
      {!loaded ? <LoadingState /> : detail ? <div className="detail-stack">
        ...
        <div className="observation-record-list"><Panel className="observation-record-item observation"><Badge tone="blue">观察</Badge><p>{detail.item.teacher_observation}</p></Panel><Panel className="observation-record-item identification"><Badge tone="green">识别</Badge><p>{detail.item.teacher_identification}</p></Panel><Panel className="observation-record-item response"><Badge tone="orange">教师原始应答</Badge><p>{detail.item.teacher_response.strategy}</p><small>复察：{detail.item.teacher_response.nextObservationFocus}</small></Panel></div>
    ...
    {entry === "web" && <Modal wide title={form.sourceImportId ? "校对AI提取的观察草稿" : "新建标准观察"} description="上传模板与网页填写使用同一份在线表单；教师确认保存前不会形成正式观察。" onClose={() => setEntry(null)}><form className="remote-observation-form" onSubmit={submit}>
      <fieldset className="observation-focus-choice"><legend>观察聚焦（单选）</legend><p>选择本次最需要聚焦的一类证据，AI仍会完整分析观察材料。</p><div>{Object.entries(focusCategoryLabel).map(([value, label]) => <label className={form.observationFocusCategory === value ? "selected" : ""} key={value}><input type="radio" name="observation-focus-category" value={value} checked={form.observationFocusCategory === value} onChange={() => setForm({ ...form, observationFocusCategory: value })} /><strong>{label}</strong></label>)}</div></fieldset>
      {form.sourceImportId && importResult && <div className="import-review-banner"><div><Badge tone="orange">模板解析草稿</Badge><strong>{importResult.source_file_name}</strong><span>请逐项核对黄色低置信度字段、观察教师和幼儿匹配，确认后再保存。</span></div><div className="import-confidence-grid">{Object.entries(importResult.field_confidence ?? {}).map(([key, value]) => <span className={value < 0.7 ? "low" : "ok"} key={key}>{key} {Math.round(value * 100)}%</span>)}</div>{importResult.extracted_fields.warnings?.map((warning) => <p className="warning-card" key={warning}>{warning}</p>)}{(importResult.extracted_fields.subjects ?? []).filter((subject) => !classChildren.some((child) => child.display_name.trim() === subject.displayName.trim())).map((subject) => <p className="warning-card" key={subject.displayName}>“{subject.displayName}”未匹配到当前班级幼儿，请在下方手动选择。</p>)}</div>}
...
export function RemoteCurriculumV32Page({ user }: { user: RemoteUser }) {
  ...
  return <div className="page remote-page evo-page"><PageHeader eyebrow="证据选取 · 课程方向 · 深度计划 · N循环" title="游戏课程生成" description="正式班本课程以共同兴趣和连续证据为起点。AI先给4个“课程题目+建议理由”，教师选择后再生成完整课程计划。" actions={<div className="page-action-row"><select className="child-select" value={classroomId} onChange={(event) => { setClassroomId(event.target.value); setSelectedId(""); }}>{classrooms.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select>{user.role === "researcher" && <button className="btn btn-secondary" onClick={() => setTemplateModal(true)}><BookOpen />模板版本</button>}<button className="btn btn-secondary" disabled={busy} onClick={() => void run(async () => { await remoteApi.scanCurriculum(classroomId); await load(); })}><BrainCircuit />语义扫描</button><button className="btn btn-primary" onClick={() => setEvidenceModal(true)}><Plus />教师选证生成</button></div>} />
    ...
    {!loaded ? <LoadingState /> : !selected ? <EmptyState title="尚无课程线索" description="先积累教师已确认的观察，再由教师选择证据或运行语义扫描。" action={<button className="btn btn-primary" onClick={() => setEvidenceModal(true)}>选择课程证据</button>} /> : <div className="master-detail"><Panel className="master-list" title="课程线索">{visible.map((item) => <button className={item.id === selected.id ? "selected" : ""} key={item.id} onClick={() => setSelectedId(item.id)}><div><Badge tone={item.threshold_met ? "green" : "orange"}>{item.threshold_met ? "达到门槛" : "继续观察"}</Badge><strong>{item.title}</strong><span>{item.child_ids.length}名幼儿 · {item.time_point_count}个时间点 · {item.evidence_observation_ids.length}条证据</span></div></button>)}</Panel><div className="detail-stack">
      ...
      <Panel className="curriculum-delete-row"><div><strong>课程线索管理</strong><small>删除只影响本线索及派生课程内容，不会删除来源观察。</small></div><button className="btn btn-danger" disabled={busy} onClick={deleteCurriculum}><Trash2 />删除课程线索</button></Panel>
      <Panel title="第一步：4个可选课程方向" subtitle="本阶段只比较课程题目和建议理由，避免过早把课程路径固定化。" action={<button className="btn btn-secondary" disabled={busy} onClick={() => void run(async () => { await remoteApi.generateCurriculumOptions(selected.id); await loadWorkspace(selected.id); })}><Sparkles />生成/更新4个方向</button>}>
        {workspace?.options.length ? <div className="evo-course-options simple-directions">{workspace.options.map((option) => <article className={selectedOptions.includes(option.id) ? "selected" : ""} key={option.id}><label><input type="checkbox" checked={selectedOptions.includes(option.id)} onChange={() => setSelectedOptions((current) => current.includes(option.id) ? current.filter((id) => id !== option.id) : current.length < 3 ? [...current, option.id] : current)} /><strong>{option.title}</strong></label><p><b>建议理由：</b>{option.value_point}</p><small>关联{option.evidence_observation_ids.length}条连续观察证据</small></article>)}</div> : <EmptyState title="尚未生成课程方向" description="AI会整合教师选定证据中的持续兴趣和未解决问题，生成4个题目与建议理由。" />}
...
export function RemoteProfessionalMemoryPage({ user }: { user: RemoteUser }) {
  ...
  const [resourcePackages, setResourcePackages] = useState<RemoteCurriculumResourcePackage[]>([]);
  const [resourceModal, setResourceModal] = useState(false);
  const [revisionPackage, setRevisionPackage] = useState<RemoteCurriculumResourcePackage | null>(null);
  const [reviewPackage, setReviewPackage] = useState<RemoteCurriculumResourcePackage | null>(null);
  const [reviewComment, setReviewComment] = useState("");
  const [resourceForm, setResourceForm] = useState({ title: "", summary: "", grades: ["middle"], themes: "" });
  const [resourceFiles, setResourceFiles] = useState<{ plan: File | null; materials: File | null; booklet: File | null; supplements: File[] }>({ plan: null, materials: null, booklet: null, supplements: [] });
  ...
  const loadResources = async () => { const result = await remoteApi.curriculumResourcePackages(); setResourcePackages(result.items); };
  const loadAll = () => Promise.all([loadMemories(), loadFrameworks(), loadResources()]);
  const run = async (operation: () => Promise<void>) => { setBusy(true); setError(""); try { await operation(); } catch (reason) { setError(showError(reason)); } finally { setBusy(false); } };
  const submitResourcePackage = (event: FormEvent) => { event.preventDefault(); void run(async () => {
    if (!resourceFiles.plan || !resourceFiles.materials || !resourceFiles.booklet) return;
    const packageId = revisionPackage?.id ?? (await remoteApi.createCurriculumResourcePackage({ title: resourceForm.title, summary: resourceForm.summary, applicableGrades: resourceForm.grades, themes: lines(resourceForm.themes) })).item.id;
    await remoteApi.uploadCurriculumResourceAsset(packageId, "plan", resourceFiles.plan);
    await remoteApi.uploadCurriculumResourceAsset(packageId, "materials", resourceFiles.materials);
    await remoteApi.uploadCurriculumResourceAsset(packageId, "booklet", resourceFiles.booklet);
    for (const file of resourceFiles.supplements) await remoteApi.uploadCurriculumResourceAsset(packageId, "supplement", file);
    await remoteApi.submitCurriculumResourcePackage(packageId);
    setResourceModal(false); setRevisionPackage(null); setResourceForm({ title: "", summary: "", grades: ["middle"], themes: "" }); setResourceFiles({ plan: null, materials: null, booklet: null, supplements: [] });
    await loadResources();
  }); };
  useEffect(() => { loadAll().catch((reason) => setError(showError(reason))).finally(() => setLoaded(true)); }, []);
  const resourceStatus: Record<RemoteCurriculumResourcePackage["status"], string> = { draft: "上传未完成", pending: "待教研审核", active: "已纳入经验库", rejected: "已退回", disabled: "已停用" };
  const resourceAssetLabel = { plan: "课程计划", materials: "课程材料", booklet: "课程手册", supplement: "补充资料" } as const;
  return <div className="page remote-page evo-page experience-page"><PageHeader eyebrow="园本沉淀 · 证据回链" title="园所经验库" description="沉淀教师确认的专业经验与教研审核通过的游戏课程资源包，供后续观察、应答和课程设计参考；不用于训练通用模型。" actions={<div className="page-action-row"><button className="btn btn-secondary" disabled={busy} onClick={() => void run(async () => { await loadAll(); })}><RefreshCcw />刷新</button><button className="btn btn-primary" onClick={() => setResourceModal(true)}><Upload />上传游戏课程资源包</button></div>} />
    {error && <div className="remote-error"><CircleAlert />{error}</div>}
    {!loaded ? <LoadingState /> : <div className="experience-content-stack">
      <Panel title="游戏课程资源包" subtitle="教师提交三类必需文件；教研员只审核这类园本资源。审核通过后，文本内容会进入AI园本参考，原文件仍可下载核对。"><div className="curriculum-resource-grid">{resourcePackages.length === 0 ? <EmptyState title="还没有游戏课程资源包" description="上传课程计划、课程材料PDF和课程手册PDF，形成可追溯的园本课程资料。" /> : resourcePackages.map((item) => <article className={`curriculum-resource-card ${item.status}`} key={item.id}><header><div><Badge tone={item.status === "active" ? "green" : item.status === "rejected" ? "red" : "orange"}>{resourceStatus[item.status]}</Badge><h3>{item.title}</h3></div><small>{item.creator_name} · {new Date(item.created_at).toLocaleDateString("zh-CN")}</small></header><p>{item.summary}</p><div className="resource-tags">{item.applicable_grades.map((grade) => <span key={grade}>{gradeLabel[grade]}</span>)}{item.themes.map((theme) => <span key={theme}>#{theme}</span>)}</div><div className="resource-assets">{item.assets.map((asset) => <button className="btn btn-secondary" key={asset.id} onClick={() => void run(() => remoteApi.downloadCurriculumResourceAsset(asset.id, asset.file_name))}><FileText />{resourceAssetLabel[asset.asset_type]}：{asset.file_name}</button>)}</div>{item.review_comment && <p className={item.status === "rejected" ? "resource-review-note rejected" : "resource-review-note"}><b>教研意见：</b>{item.review_comment}</p>}{item.status === "rejected" && item.created_by === user.id && <footer><button className="btn btn-primary" onClick={() => { setRevisionPackage(item); setResourceForm({ title: item.title, summary: item.summary, grades: item.applicable_grades, themes: item.themes.join("，") }); setResourceFiles({ plan: null, materials: null, booklet: null, supplements: [] }); setResourceModal(true); }}><Upload />替换文件并重新提交</button></footer>}{user.role === "researcher" && item.status === "pending" && <footer><button className="btn btn-primary" disabled={busy} onClick={() => void run(async () => { await remoteApi.reviewCurriculumResourcePackage(item.id, { decision: "active", comment: "教研审核通过" }); await Promise.all([loadResources(), loadMemories()]); })}><CheckCircle2 />审核通过并纳入AI参考</button><button className="btn btn-secondary" onClick={() => { setReviewPackage(item); setReviewComment(""); }}>退回修改</button></footer>}</article>)}</div></Panel>
      <Panel title="园本分析框架" subtitle="框架用于约束AI观察角度和证据提醒；不生成总分、排名或人格标签。" action={user.role === "researcher" ? <button className="btn btn-secondary" onClick={() => setFrameworkModal(true)}><Plus />发布新版本</button> : undefined}><div className="evo-framework-list">{frameworks.filter((item) => item.is_default).map((item) => <article key={item.id}><div><Badge tone="blue">{item.framework_type === "game_experience" ? "游戏经验" : "学习品质"}</Badge><strong>{item.name} · V{item.version}</strong></div><p>{item.description}</p><small>{item.dimensions.map((dimension) => dimension.label).join(" · ")}</small></article>)}</div></Panel>
      {items.length === 0 ? <EmptyState title="暂无可用园本经验" description="完成教师确认、应答复察、课程复盘或资源包审核后，系统会形成带证据来源的园本经验。" /> : <div className="evo-memory-grid">{items.map((item) => <Panel key={item.id}><div className="evo-section-head"><div><Badge tone="green">{memoryTypeLabel[item.memory_type]}</Badge><h3>{item.title}</h3></div><span className="memory-source-tag">可用于AI参考</span></div><p>{item.summary}</p><details><summary>查看检索文本与证据来源</summary><p>{item.retrieval_text}</p><small>来源：{item.source_resource_type} · {item.source_resource_id}</small></details></Panel>)}</div>}
    </div>}
    {resourceModal && <Modal wide title={revisionPackage ? "替换退回资源并重新提交" : "上传游戏课程资源包"} description={revisionPackage ? "资源包说明保持不变，请按教研意见替换三类必需文件。" : "三类必需文件齐全后会提交教研审核；审核前不会进入AI参考。"} onClose={() => { setResourceModal(false); setRevisionPackage(null); }}><form className="remote-form resource-upload-form" onSubmit={submitResourcePackage}><label><span>资源包名称</span><input required disabled={Boolean(revisionPackage)} minLength={2} value={resourceForm.title} onChange={(event) => setResourceForm({ ...resourceForm, title: event.target.value })} /></label><label className="full-field"><span>内容摘要与适用说明</span><textarea required disabled={Boolean(revisionPackage)} minLength={2} rows={4} value={resourceForm.summary} onChange={(event) => setResourceForm({ ...resourceForm, summary: event.target.value })} /></label><fieldset className="full-field" disabled={Boolean(revisionPackage)}><legend>适用年龄段</legend>{(["small", "middle", "large"] as const).map((grade) => <label key={grade}><input type="checkbox" checked={resourceForm.grades.includes(grade)} onChange={() => setResourceForm({ ...resourceForm, grades: resourceForm.grades.includes(grade) ? resourceForm.grades.filter((item) => item !== grade) : [...resourceForm.grades, grade] })} />{gradeLabel[grade]}</label>)}</fieldset><label className="full-field"><span>相关游戏主题（逗号或换行分隔）</span><textarea disabled={Boolean(revisionPackage)} rows={2} value={resourceForm.themes} onChange={(event) => setResourceForm({ ...resourceForm, themes: event.target.value })} /></label><label><span>1. 游戏课程计划（DOC/DOCX，必需）</span><input required type="file" accept=".doc,.docx" onChange={(event) => setResourceFiles({ ...resourceFiles, plan: event.target.files?.[0] ?? null })} /></label><label><span>2. 游戏课程材料（PDF，必需）</span><input required type="file" accept="application/pdf,.pdf" onChange={(event) => setResourceFiles({ ...resourceFiles, materials: event.target.files?.[0] ?? null })} /></label><label><span>3. 游戏课程手册（PDF，必需）</span><input required type="file" accept="application/pdf,.pdf" onChange={(event) => setResourceFiles({ ...resourceFiles, booklet: event.target.files?.[0] ?? null })} /></label><label><span>补充资料（可选，最多5个）</span><input type="file" multiple accept=".doc,.docx,.pdf,.jpg,.jpeg,.png" onChange={(event) => setResourceFiles({ ...resourceFiles, supplements: Array.from(event.target.files ?? []).slice(0, 5) })} /></label><p className="full-field form-note">单文件不超过30MB。课程材料与手册采用PDF，确保不同设备上版式稳定。</p><button className="btn btn-primary" disabled={busy || !resourceFiles.plan || !resourceFiles.materials || !resourceFiles.booklet || resourceForm.grades.length === 0}><Upload />{revisionPackage ? "替换文件并重新提交" : "上传并提交教研审核"}</button></form></Modal>}
    {reviewPackage && <Modal title={`退回：${reviewPackage.title}`} description="请写明需要教师补充或调整的具体内容。" onClose={() => setReviewPackage(null)}><div className="remote-form"><label><span>教研修改说明</span><textarea required minLength={2} rows={5} value={reviewComment} onChange={(event) => setReviewComment(event.target.value)} /></label><button className="btn btn-primary" disabled={busy || reviewComment.trim().length < 2} onClick={() => void run(async () => { await remoteApi.reviewCurriculumResourcePackage(reviewPackage.id, { decision: "rejected", comment: reviewComment }); setReviewPackage(null); await loadResources(); })}>确认退回</button></div></Modal>}

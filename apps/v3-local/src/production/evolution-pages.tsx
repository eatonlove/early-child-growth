...
const statusLabel: Record<string, string> = {
  submitted: "教师已提交", ai_ready: "待教师审核", adopted: "教师已采用", abandoned: "已放弃",
  pending: "待审核", suggested: "AI候选", planned: "待实施", implemented: "已实施", follow_up: "待复察",
  verified: "已验证", rejected: "未选择", clue: "课程线索", draft: "草案", reviewed: "已审核", active: "实施中", reflected: "已复盘",
};
...
export function RemoteObservationV32Page({ user }: { user: RemoteUser }) {
  ...
  const [exportForm, setExportForm] = useState({ variant: "teacher" as "teacher" | "professional", purpose: "园内教研与档案留存", recipient: "本园教研组" });
  ...
  return <div className="page remote-page evo-page">
    ...
    <div className="evo-flow"><span>1 教师记录</span><ArrowRight /><span>2 AI分幼儿分析</span><ArrowRight /><span>3 教师板块审核</span><ArrowRight /><span>4 选择应答并复察</span></div>
    <div className="remote-observation-layout">
      ...
      {!loaded ? <LoadingState /> : detail ? <div className="detail-stack">
        ...
        {latest && <AnalysisBoard analysis={latest} childName={childMap.get(analysisChildId)?.display_name ?? "幼儿"} responsePlans={responsePlans} evidence={detail.evidence} busy={busy} onRefresh={refreshDetail} onError={setError} />}
    ...
    {exportModal && detail && <Modal title="申请观察记录Word" description="文档只包含教师已确认内容；含幼儿信息，提交后由教研员审批。" onClose={() => setExportModal(false)}><form className="remote-form" onSubmit={(event) => { event.preventDefault(); void withBusy(async () => { await remoteApi.requestObservationDocument(detail.item.id, exportForm); setExportModal(false); }); }}><label><span>版本</span><select value={exportForm.variant} onChange={(event) => setExportForm({ ...exportForm, variant: event.target.value as "teacher" | "professional" })}><option value="teacher">教师原稿版</option><option value="professional">教师终审专业版</option></select></label><label><span>用途</span><textarea required value={exportForm.purpose} onChange={(event) => setExportForm({ ...exportForm, purpose: event.target.value })} /></label><label><span>接收方</span><input required value={exportForm.recipient} onChange={(event) => setExportForm({ ...exportForm, recipient: event.target.value })} /></label><button className="btn btn-primary" disabled={busy}><FileText />提交导出审批</button></form></Modal>}
  </div>;
}
...
export function RemoteCurriculumV32Page({ user }: { user: RemoteUser }) {
  ...
  const [evidenceModal, setEvidenceModal] = useState(false); const [cycleModal, setCycleModal] = useState(false); const [exportModal, setExportModal] = useState(false); const [templateModal, setTemplateModal] = useState(false);
  ...
  const [exportForm, setExportForm] = useState({ purpose: "园内课程审议与档案留存", recipient: "本园教研组" }); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [loaded, setLoaded] = useState(false);
  ...
  return <div className="page remote-page evo-page"><PageHeader eyebrow="证据选取 · 活动方向 · 课程生成 · N循环" title="游戏课程生成" description="正式班本课程以共同兴趣和连续证据为起点。AI先给4个方向，教师选择1至3个后再生成园本课程计划。" actions={<div className="page-action-row"><select className="child-select" value={classroomId} onChange={(event) => { setClassroomId(event.target.value); setSelectedId(""); }}>{classrooms.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select>{user.role === "researcher" && <button className="btn btn-secondary" onClick={() => setTemplateModal(true)}><BookOpen />模板版本</button>}<button className="btn btn-secondary" disabled={busy} onClick={() => void run(async () => { await remoteApi.scanCurriculum(classroomId); await load(); })}><BrainCircuit />语义扫描</button><button className="btn btn-primary" onClick={() => setEvidenceModal(true)}><Plus />教师选证生成</button></div>} />
    ...
    {!loaded ? <LoadingState /> : !selected ? <EmptyState title="尚无课程线索" description="先积累已终审观察，再由教师选择证据或运行语义扫描。" action={<button className="btn btn-primary" onClick={() => setEvidenceModal(true)}>选择课程证据</button>} /> : <div className="master-detail"><Panel className="master-list" title="课程线索">{visible.map((item) => <button className={item.id === selected.id ? "selected" : ""} key={item.id} onClick={() => setSelectedId(item.id)}><div><Badge tone={item.threshold_met ? "green" : "orange"}>{item.threshold_met ? "达到门槛" : "继续观察"}</Badge><strong>{item.title}</strong><span>{item.child_ids.length}名幼儿 · {item.time_point_count}个时间点 · {item.evidence_observation_ids.length}条证据</span></div></button>)}</Panel><div className="detail-stack">
    ...
    {exportModal && latestPlan && <Modal title="申请课程计划Word" description="课程文档包含已选证据和当前N轮记录，审批通过后可下载。" onClose={() => setExportModal(false)}><form className="remote-form" onSubmit={(event) => { event.preventDefault(); void run(async () => { await remoteApi.requestCurriculumDocument(latestPlan.id, exportForm); setExportModal(false); }); }}><label><span>用途</span><textarea value={exportForm.purpose} onChange={(event) => setExportForm({ ...exportForm, purpose: event.target.value })} /></label><label><span>接收方</span><input value={exportForm.recipient} onChange={(event) => setExportForm({ ...exportForm, recipient: event.target.value })} /></label><button className="btn btn-primary" disabled={busy}><FileText />提交导出审批</button></form></Modal>}
...
export function RemoteProfessionalMemoryPage({ user }: { user: RemoteUser }) {
  ...
  const [filter, setFilter] = useState<"pending" | "active" | "disabled">(user.role === "researcher" ? "pending" : "active");
  ...
  const loadMemories = async (status = filter) => { const result = await remoteApi.professionalMemories(user.role === "researcher" ? status : "active"); setItems(result.items); };
  const loadFrameworks = async () => { const result = await remoteApi.analysisFrameworks(); setFrameworks(result.items); };
  useEffect(() => { Promise.all([loadMemories(), loadFrameworks()]).catch((reason) => setError(showError(reason))).finally(() => setLoaded(true)); }, [filter, user.role]);
  const decide = (id: string, status: "active" | "disabled", qualityScore?: number) => void (async () => { setBusy(true); setError(""); try { await remoteApi.updateProfessionalMemory(id, { status, qualityScore }); await loadMemories(); } catch (reason) { setError(showError(reason)); } finally { setBusy(false); } })();
  return <div className="page remote-page evo-page"><PageHeader eyebrow="人工审核 · 园本沉淀 · 可追溯停用" title="园所专业经验库" description="教师对AI的有效修订、经复察验证的应答效果和课程反思，经教研员审核后才进入后续AI检索参考；不用于训练通用模型。" actions={user.role === "researcher" ? <div className="page-action-row"><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="pending">待审核</option><option value="active">已启用</option><option value="disabled">已停用</option></select><button className="btn btn-secondary" onClick={() => void loadMemories()}><RefreshCcw />刷新</button></div> : undefined} />
    {error && <div className="remote-error"><CircleAlert />{error}</div>}
    {!loaded ? <LoadingState /> : <><Panel title="园本分析框架" subtitle="框架用于约束AI观察角度和证据提醒；不生成总分、排名或人格标签。" action={user.role === "researcher" ? <button className="btn btn-secondary" onClick={() => setFrameworkModal(true)}><Plus />发布新版本</button> : undefined}><div className="evo-framework-list">{frameworks.filter((item) => item.is_default).map((item) => <article key={item.id}><div><Badge tone="blue">{item.framework_type === "game_experience" ? "游戏经验" : "学习品质"}</Badge><strong>{item.name} · V{item.version}</strong></div><p>{item.description}</p><small>{item.dimensions.map((dimension) => dimension.label).join(" · ")}</small></article>)}</div></Panel>{items.length === 0 ? <EmptyState title={filter === "pending" ? "暂无待审核经验" : "暂无可用园本经验"} description="完成AI终审、应答复察或课程复盘后，系统会形成带证据来源的待审核条目。" /> : <div className="evo-memory-grid">{items.map((item) => <Panel key={item.id}><div className="evo-section-head"><div><Badge tone={item.status === "active" ? "green" : item.status === "pending" ? "orange" : "gray"}>{memoryTypeLabel[item.memory_type]}</Badge><h3>{item.title}</h3></div><strong>{Math.round(item.quality_score * 100)}%</strong></div><p>{item.summary}</p><details><summary>查看检索文本与证据来源</summary><p>{item.retrieval_text}</p><small>来源：{item.source_resource_type} · {item.source_resource_id}</small></details>{user.role === "researcher" && <div className="evo-review-actions">{item.status !== "active" && <button className="btn btn-primary" disabled={busy} onClick={() => decide(item.id, "active", Math.max(item.quality_score, 0.7))}><Check />审核启用</button>}{item.status !== "disabled" && <button className="btn btn-secondary" disabled={busy} onClick={() => decide(item.id, "disabled")}>停用</button>}</div>}</Panel>)}</div>}</>}

...
begin
  ...
  then
    ...
    execute $trigger$
      ...
      for each row
      when (
        coalesce(new.raw_app_meta_data ->> 'application', '') <> 'tongji_v3'
        and lower(coalesce(new.email, '')) not like '%@tongji-v3.local'
      )
...
comment on schema tongji_v3 is
  '童迹3.0业务schema；共享Auth触发器按应用标记及童迹专用内部邮箱域名隔离其他应用资料写入。';

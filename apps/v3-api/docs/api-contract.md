...
后端会根据 `exportType + resourceId` 再次校验对象存在性、访问权限和关联班级，并自行写入规范化的 `resourceType`，不接受前端手填对象类型或班级。当前支持个体报告、课程案例和匿名教研数据；班级报告尚未形成真实报告对象，因此不提供新申请入口。审批通过不自动打包文件，也不扩大使用范围；系统记录申请对象、用途、接收方、去标识要求和决定。
